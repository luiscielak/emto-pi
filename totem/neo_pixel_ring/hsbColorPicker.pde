...
//      print("pickedB = " + pickedB + "("+ x + ", " + y + ")\n");
  ...
  float getHue(color thisColor) {
    return hue(thisColor)*(255.0/MAX_HUE);
  }

  float getSaturation(color thisColor) {
    return saturation(thisColor)*(255.0/MAX_SATURATION);
  }

  float getBrightness(color thisColor) {
    return brightness(thisColor)*(255.0/MAX_BRIGHTNESS);
  }
}
